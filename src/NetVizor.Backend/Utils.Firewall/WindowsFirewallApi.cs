    ...
    public bool CreateRule(CreateRuleRequest request)
    {
        try
        {
            ...
            if ((int)request.Protocol == 1) // ICMP
            {
                ...
                newRule.RemotePorts = null;
                if (!string.IsNullOrWhiteSpace(request.IcmpTypesAndCodes))
                {
                    newRule.IcmpTypesAndCodes = request.IcmpTypesAndCodes;
                }
            }
            else
            {
                // 非 ICMP 时不要设置 IcmpTypesAndCodes，否则抛异常
                // 仅在 Protocol == 1 时有效，否则抛出异常
                // newRule.IcmpTypesAndCodes = null;
                if (!string.IsNullOrWhiteSpace(request.LocalPorts))
                {
        ...
        {
            var format = ex.ToString();
            Console.WriteLine(format); // 包含类型、消息、堆栈跟踪
            return false;
        }
    }
