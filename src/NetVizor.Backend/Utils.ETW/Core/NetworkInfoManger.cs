...
public sealed class NetworkInfoManger
{
    ...
    public NetworkModel? GetUdpCache(NetworkModel value)
    {
        return GetUdpCache(value.GetKey());
    }

    public NetworkModel[] GetAllUdpSessions()
    {
        return _activeUdpSessions.Values.ToArray();
    }

    public void RemoveUdpSession(NetworkModel value)
    {
        _activeUdpSessions.TryRemove(value.GetKey(), out _);
    }

    public NetworkModel? GetUdpCache(string key)
    {
        return _activeUdpSessions[key];
    }
